...
;(function ($) {
    ...
    var handleSignature = function () {
        ...
        LiberSign.sign(selectedCert, config.signatureInformations).then(function (signatures) {
            ...
            that.find('#libersign-message').text(error.exception || "Erreur inconnue. Merci de contacter votre administrateur.");
            window.removeEventListener('libersignmessage', handleMessage);
        });
    };
    ...
    var handleLoadedCerts = function () {
        ...
        certSelector += '<span style="float:right;"><button class="btn btn-warning" id="libersign-cancel"><i class="fa fa-times-circle-o"></i> Annuler</button>';
        certSelector += '<button class="btn btn-primary" id="libersign-confirm" disabled><i class="fa ls-signature"></i> Signer</button></span>';
    ...
    var constructCertificateRow = function (cert) {
        ...
        } else if (cert.verifiedWith.indexOf('EIDAS') != -1) {
            certSelector += '<span class="libersign-tooltip label label-default">eIDAS</span>'
        } else if (cert.verifiedWith.indexOf('ADULLACT') != -1) {
            certSelector += '<span class="libersign-tooltip label label-info">Adullact</span>'
        } else if (cert.verifiedWith.indexOf('LIBRICIEL') != -1) {
            certSelector += '<span class="libersign-tooltip label label-primary">Libriciel</span>'
        }
    ...
    var handleExtension = function () {
        ...
        return '<div  class="libersign-loader"></div>' +
            ...
            '<span id="libersign-error" style="display:none;" class="text-danger"><i class="' + config.iconType + ' ' + icons[config.iconType].warn + '"></i> Erreur lors de la signature.<br/><span id="libersign-message"></span><br/><button id="libersign-error-confirm" class="btn btn-default"><i class="fa fa-arrow-left"></i> Retour</button></span>' +
    ...
    var testForLiberSign = function () {
        return new Promise(function(resolve, reject) {
            ...
            LiberSign.getVersion().then(function () {
                resolve();
            }).catch(function () {
                reject();
            });
        ...
        init: function () {
            ...
            this.filter("div").each(function () {
                ...
                if (typeof LiberSign === 'object') {
                    var that = this;
                    testForLiberSign().then(function() {
                        block.append(handleExtension.apply(that));
                    }).catch(function() {
                        block.append(handleError.apply(that));
                    });
                } else {
